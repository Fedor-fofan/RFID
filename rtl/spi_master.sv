module spi_master(
	input logic clk,
    input logic rst,

    input logic [7:0] spi_tx_data,
    input logic spi_tx_valid,
    output logic spi_tx_ready,

    output logic [7:0] spi_rx_data,
    input logic spi_rx_ready, 
    output logic spi_rx_valid, 

    input logic spi_start,
    input logic spi_last,
    output logic spi_busy,
    output logic spi_done,

    output logic spi_mosi, 
    input logic spi_miso,
    output logic spi_clk,
    output logic spi_cs
);
	logic [3:0] counter; 
	logic spi_clk_rst;
	logic [7:0] spi_tx_data_r;
	
	enum logic [1:0] { 
		IDLE, 
		SPI_WAIT, 
		SPI_SHIFT,
		SPI_DONE
	} state, next_state;

	spi_clock spi_clk_i(
		.clk(clk&(state==SPI_SHIFT)),
		.rst(rst|spi_clk_rst),
		.spi_clk(spi_clk)
	);

	always_comb begin
		next_state = state;

		spi_tx_ready = 'b1;
		spi_rx_valid = counter==4'd8;
		spi_busy = 'b0;
		spi_done = 'b0;
		spi_cs = 1'b1;
		spi_clk_rst = 1'b0;

		case(state)
			IDLE: if (spi_start) next_state = SPI_WAIT; 
			SPI_WAIT: begin
				spi_cs = 1'b0;
				spi_clk_rst = 1'b1;

				if (spi_tx_valid&spi_rx_ready) next_state = SPI_SHIFT;
			end
			SPI_SHIFT: begin
				spi_tx_ready = 1'b0;
				spi_busy = 1'b1;
				spi_cs = 1'b0;

				if (counter>=4'd8) next_state = spi_last ? SPI_DONE : SPI_WAIT;
			end
			SPI_DONE: begin
				spi_done = 1'b1;
				spi_clk_rst = 1'b1;

				next_state = IDLE;
			end
		endcase
	end

	always_ff @(posedge clk) begin
		if(rst) begin
			state <= IDLE;
			counter <= '0;
			spi_tx_data_r <= '0;
		end else begin
			state <= next_state;
			if (state!=SPI_SHIFT && counter!=0) counter <= 0;
			if (state==SPI_WAIT && spi_tx_ready) spi_tx_data_r <= spi_tx_data;
		end
	end

	always_ff @( posedge spi_clk ) begin
		if (state==SPI_SHIFT && counter<4'd8) begin
			$display("counter: %d spi_done: %b spi_rx_data: %b", counter, spi_done, spi_rx_data);
			{spi_mosi, spi_tx_data_r} <= {spi_tx_data_r, spi_rx_data[7]};
			spi_rx_data <= {spi_rx_data[6:0],spi_miso};
			counter <= counter + 1;
		end
	end

endmodule